...
namespace APIGateway.Services
{
    public class DocumentManagement : IDocumentManagement
    {
        ...
        public async Task<string> UploadFile(string foldername, Byte[] file, string contentType)
        {
            var content =new ByteArrayContent(file);
            var response = await _httpClient.PutAsync($"/api/Document/upload?foldername={foldername}?contentType={contentType}", content);
            response.EnsureSuccessStatusCode();
        ...
        //public async Task<Byte[]> GeneratePDF(ProposalLetter proposal, string sign)
    }
}